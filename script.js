...
// FUNCTIONS TO GENERATE DIFFERENT PALETTES
...
function generateAnalogousPalette(hsl, count) {
  ...
  // generate colors specified in count
  for (let i = 0; i < count; i++) {
    // add 30 and multiply i for every color
    // This is a common approach to create analogous colors
    // since it increments the hue in steps of 30, keeping the colors adjacent on the color wheel.
    let newHue = hue + 30 * i;

    // if new hue is greater than 360, -360 from it
    if (newHue > 360) {
      newHue -= 360;
    }
  ...
  // return palette array after getting all colors
  return palette;
}
...
function generateShadesPalette(hsl, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // new saturation cannot be greater than 100
...
function generateRelatedColorPalette(hsl, count) {
  ...
  // to get related colors, we will play around with the hue, saturation and lightness
  // increase saturation by 20 and if greater than 100 reduce
  palette.push([hue, (saturation + 20) % 100, lightness]);
  // decrease saturation by 20 and if greater than 100 reduce
  palette.push([hue, (saturation - 20) % 100, lightness]);

  // increase lightness by 20 and if greater than 100 reduce
  palette.push([hue, saturation, (lightness + 20) % 100]);
  // decrease lightness by 20 and if greater than 100 reduce
  palette.push([hue, saturation, (lightness - 20) % 100]);

  // increase hue by 20 and if greater than 100 reduce
  palette.push([(hue + 20) % 100, saturation, lightness]);
  // decrease hue by 20 and if greater than 100 reduce
  palette.push([(hue - 20) % 100, saturation, lightness]);

  // shuffle palette array
...
// Function to call a specific function above based on specific palette type
...
function generatePaletteHtml(type, container) {
  ...
  let palette = [];
  container.innerHTML = "";
  // if type is image colours, no need to generate palette because we have imageColors array
  ...
  palette.forEach((color) => {
    // if type image colors, it already has hex colors so no need to convert
    if (type != "image-colors") {
...
// Function to get hsl from any type of given color
function getHslFromcolor(color) {
  let hsl;
  if (isValidColor(color)) {
    // if valid color name, hex, rgb given
    // create a temp div element give it color and get that color from that div which will be rgb always then we can convert rgb to hsl
    ...
    // no need of temp div anymore, we can remove it
...
// Function to check if a given value is valid CSS
function isValidColor(color) {
  // check color validity
  return CSS.supports("color", color);
}

// Function that changes the string "rgb(255,255,255)" to an array ["255","255","255"]
...
// change preview palette and generate palette when a color is written in the input
searchInput.addEventListener("keyup", (e) => {
  const value = e.target.value;
  // if valid color is given
...
function toast(message) {
  ...
  // insert the message to the div element
  toast.textContent = message;
  // append div element to document body
  document.body.appendChild(toast);

  // show the div element to animate
  setTimeout(() => {
    toast.classList.add("show");
  }, 10);

  // remove div element after 2 seconds
  setTimeout(() => {
